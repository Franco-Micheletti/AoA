...
import { createPlayerGames } from '../../../../prisma/client/games/createNewGames'
...
export async function POST (request) {
  ...
  const profiles = gamesData.profiles
  if (games !== null && games !== undefined) {
    if (games.length > 0) {
      ...
      const status = await createPlayerGames(cleanData, profiles)
